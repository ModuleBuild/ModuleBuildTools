...
if ((Get-Variable 'BuildEnv' -ErrorAction:SilentlyContinue) -eq $null) {
    $Script:BuildEnv = New-Object -TypeName PSObject -Property @{
        ...
        Encoding = 'utf8'
        ModuleToBuild = 'ModuleBuildTools'
        ModuleVersion = '0.0.1'
        ModuleWebsite = 'https://github.com/ModuleBuild/ModuleBuildTools'
        ModuleCopyright = "(c) $((get-date).Year.ToString()) Justin Perdok. All rights reserved."
        ModuleLicenseURI = 'https://github.com/ModuleBuild/ModuleBuildTools/LICENSE.md'
        ModuleTags = 'scaffold, Module, Invoke-Build, ModuleBuild' -split ','
        ModuleAuthor = 'Justin Perdok'
        ModuleDescription = 'Build tools for the ModuleBuild Project'
        ...
        # You should NOT enter this key here but rather manually enter it in the ModuleBuildTools.buildenvironment.json file with: Set-MBBuildEnvironment -NugetAPIKey '<key>'
    ...
    ########################################
    $PersistentBuildFile = join-path $PSScriptRoot "ModuleBuildTools.buildenvironment.json"
